...
template <typename kernel_weightDataType, typename image_DataType,typename image_DataType_out,typename image_DataType_Inter,typename convInternProd_datatype>
//image_DataType_out*
  void convolution_2D_fixed(kernel_weightDataType *kernel, image_DataType *image, image_DataType_out* image_out,const unsigned char CNNCONVLAYER_CHANNEL_IN_NUM,const unsigned char KERNELCONVLAYER_CHANNEL_IN_NUM,const unsigned char IMG_SIZE,const unsigned char KERNEL_SIZE,std::ofstream &file)
{

    image_DataType* image_Add_zero = new image_DataType[(IMG_SIZE+2)*(IMG_SIZE+2)];

  image_DataType_Inter out1 = 0;
  convInternProd_datatype prod=0;
...
template <typename kernel_weightDataType, typename image_DataType,typename image_DataType_out,typename image_DataType_Inter,typename bias_type,typename convInternProd_datatype, typename beforeReluOut_datatype>
//image_DataType_out*
  void convolution_3D_fixed(kernel_weightDataType *kernel, image_DataType *image,image_DataType_out* image_out, bias_type* bias , const unsigned char CNNCONVLAYER_CHANNEL_IN_NUM,const unsigned char KERNELCONVLAYER_CHANNEL_IN_NUM,const unsigned char IMG_SIZE,const unsigned char KERNEL_SIZE, std::ofstream &file){
    ...
    beforeReluOut_datatype* out=new  beforeReluOut_datatype[IMG_SIZE*IMG_SIZE*KERNELCONVLAYER_CHANNEL_IN_NUM];
   ...
   for (int i=0;i < KERNELCONVLAYER_CHANNEL_IN_NUM;i++)
    {
	  ...
	  //cout <<"nb image ="<< j << endl;
	    ...
	    convolution_2D_fixed<kernel_weightDataType,image_DataType,image_DataType_out,image_DataType_Inter,
	      convInternProd_datatype>(kernel1[j],image1[j], image1_out[j],CNNCONVLAYER_CHANNEL_IN_NUM,
				       KERNELCONVLAYER_CHANNEL_IN_NUM,IMG_SIZE, KERNEL_SIZE,file);
        ...
        image_out[i*IMG_SIZE*IMG_SIZE+c]=relu<image_DataType_out>(add_bias<beforeReluOut_datatype>(out[i*IMG_SIZE*IMG_SIZE+c],bias[i]));

	  /* for test */
	  //file<<"pixel n°" << i*IMG_SIZE*IMG_SIZE+c << " " << out[i*IMG_SIZE*IMG_SIZE+c] << std::endl;
	// image_out[i*IMG_SIZE*IMG_SIZE+c]=out[i*IMG_SIZE*IMG_SIZE+c];
        }
    }
